...
namespace CareerPlanning.GradeLevels
{
    public partial class Seniors : System.Web.UI.Page
    {
        ...
        protected string getENum()
        {
            if (Session["username"] != null)
            {
                // ensures student is logged in before retrieving session variable 
        ...
        protected void populateCheckBoxes()
        {
            ...
            //retrieves database values for check boxes 
            string SQL = "SELECT CareerFairs, Internships, DataAnalytics, ApplyCareerPos FROM dbo.Students WHERE CAST(eNumber as VARCHAR) = @eNumber";
            ...
            //checks check boxes if student has checked them previously 
            if (reader.Read())
            {
                bool CareerFairs = reader.GetBoolean(0);
                bool Internships = reader.GetBoolean(1);
                bool DataAnalytics = reader.GetBoolean(2);
                bool ApplyCareerPos = reader.GetBoolean(3);
        ...
        //updates database if student checks check box 
        ...
        //updates database if student unchecks check box 
        ...
        protected void ddGradeLevel4_SelectedIndexChanged(object sender, EventArgs e)
        {
            // ensures selected index is valid 
            if (ddGradeLevel4.SelectedIndex != 0)
            {
                if (Session["username"] != null)
                {
                    string strConnection = ConfigurationManager.AppSettings["ConnectionString"];
                    ...
                    SqlCommand command = new SqlCommand(SQL, connection);
                    string eNumber = getENum();
                    command.Parameters.AddWithValue("@selected", ddGradeLevel4.SelectedValue.ToString());
                    command.Parameters.AddWithValue("@eNumber", eNumber);

                    command.ExecuteNonQuery();

                    // redirects to new grade level dashboard 
                    string gradeLevel = ddGradeLevel4.SelectedValue.ToString();
                    if (gradeLevel == "Fr")
                    {
        ...
        protected void cbSn1_CheckedChanged(object sender, EventArgs e)
        {
            string studentENumber = getENum();
            if (cbSn1.Checked)
            {
                updateTaskCompleted("CareerFairs");
                // used for back-end record keeping 
                AddUpdateTaskComplete("CareerFairs", studentENumber);
            ...
            {
                updateTaskNotCompleted("CareerFairs");
        ...
        protected void cbSn2_CheckedChanged(object sender, EventArgs e)
        {
            string studentENumber = getENum();
            if (cbSn2.Checked)
            {
                updateTaskCompleted("Internships");
                // used for back-end record keeping 
                AddUpdateTaskComplete("Internships", studentENumber);
                // awards points based on task completed toward achievements
                ChangeSkillLevel("SelfDevelopment", studentENumber, (GetCurrentSkillLevel("SelfDevelopment", studentENumber) + 1));
            }
            else
            {
                updateTaskNotCompleted("Internships");
                // removes points based on task completed toward achievements
                ChangeSkillLevel("SelfDevelopment", studentENumber, (GetCurrentSkillLevel("SelfDevelopment", studentENumber) - 1));
            }
        }

        protected void cbSn3_CheckedChanged(object sender, EventArgs e)
        {
            string studentENumber = getENum();
            if (cbSn3.Checked)
            {
                updateTaskCompleted("DataAnalytics");
                // used for back-end record keeping 
                AddUpdateTaskComplete("DataAnalytics", studentENumber);
                // awards points based on task completed toward achievements
                ChangeSkillLevel("SelfDevelopment", studentENumber, (GetCurrentSkillLevel("SelfDevelopment", studentENumber) + 1));
            ...
            {
                updateTaskNotCompleted("DataAnalytics");
                // removes points based on task completed toward achievements
                ChangeSkillLevel("SelfDevelopment", studentENumber, (GetCurrentSkillLevel("SelfDevelopment", studentENumber) - 1));
                ChangeSkillLevel("CriticalThinking", studentENumber, (GetCurrentSkillLevel("CriticalThinking", studentENumber) - 1));
                ChangeSkillLevel("Technology", studentENumber, (GetCurrentSkillLevel("Technology", studentENumber) - 2));
        ...
        protected void cbSn4_CheckedChanged(object sender, EventArgs e)
        {
            string studentENumber = getENum();
            if (cbSn4.Checked)
            {
                updateTaskCompleted("ApplyCareerPos");
                // used for back-end record keeping 
                AddUpdateTaskComplete("ApplyCareerPos", studentENumber);
                // awards points based on task completed toward achievements
                ChangeSkillLevel("SelfDevelopment", studentENumber, (GetCurrentSkillLevel("SelfDevelopment", studentENumber) + 1));
                ChangeSkillLevel("Communication", studentENumber, (GetCurrentSkillLevel("Communication", studentENumber) + 2));
                ChangeSkillLevel("Leadership", studentENumber, (GetCurrentSkillLevel("Leadership", studentENumber) + 1));
            }
            else
            {
                updateTaskNotCompleted("ApplyCareerPos");
                // removes points based on task completed toward achievements
                ChangeSkillLevel("SelfDevelopment", studentENumber, (GetCurrentSkillLevel("SelfDevelopment", studentENumber) - 1));
                ChangeSkillLevel("Communication", studentENumber, (GetCurrentSkillLevel("Communication", studentENumber) - 2));
                ChangeSkillLevel("Leadership", studentENumber, (GetCurrentSkillLevel("Leadership", studentENumber) - 1));
